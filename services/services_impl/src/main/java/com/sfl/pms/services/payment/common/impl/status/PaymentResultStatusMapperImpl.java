...
public class PaymentResultStatusMapperImpl implements PaymentResultStatusMapper {
    ...
    @Override
    public OrderRefundRequestState getRefundResultStatusForAcaptureRefundStatus(final String acaptureRefundStatus) {
        for (final String pattern : ACAPTURE_SUCCESSFUL_STATUS_CODE_PATTERNS) {
            if (acaptureRefundStatus.matches(pattern))
                return OrderRefundRequestState.PROCESSED;
        }
        LOGGER.warn("No known status pattern to match result code - {}", acaptureRefundStatus);
        return OrderRefundRequestState.FAILED;
    }
}
